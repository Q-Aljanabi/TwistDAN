...
def dataload():
    ...
    try:
        train_df = pd.read_csv("/train_640k.csv")
        val_df = pd.read_csv(" /valid_80k.csv")
        test_df = pd.read_csv(" /test_80k.csv")
        df_ts1 = pd.read_csv(" /TS1.csv")
        df_ts2 = pd.read_csv(" /TS2.csv")
        df_ts3 = pd.read_csv(" /TS3.csv")
        
    except FileNotFoundError as e:
        print(f"Error: {e}. Please ensure the train, valid, and test CSV files are in the specified directories.")
        return None

    print(f"Dataset shapes: Training: {train_df.shape}, Validation: {val_df.shape}, Test: {test_df.shape}")
    train_dataset = RobustMoleculeDataset(train_df, smiles_column='smiles', target_column='labels')
    val_dataset = RobustMoleculeDataset(val_df, smiles_column='smiles', target_column='labels')
    test_dataset = RobustMoleculeDataset(test_df, smiles_column='smiles', target_column='labels')
